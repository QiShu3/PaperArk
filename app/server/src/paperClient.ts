...
import { normalizeArxivId, type ArxivEntry } from './arxiv.js';
...
function hitToEntry(h: PaperHit): ArxivEntry {
  const paperId = h.paper_id ?? '';
  return {
    id: h.url ?? (paperId ? `https://arxiv.org/abs/${paperId}` : ''),
    arxivId: paperId,
    baseId: normalizeArxivId(paperId) || normalizeArxivId(h.url ?? ''),
    title: h.title ?? '',
    summary: h.abstract ?? '',
    published: h.published_date ?? '',
    updated: h.updated_date ?? undefined,
...
/**
 * 通过 paper-search-mcp 的 search_papers 搜索（固定只走 arxiv 源），
 * 返回与 searchArxiv 兼容的 ArxivEntry 列表。
 */
export async function searchEntries(query: string, maxResults: number): Promise<ArxivEntry[]> {
...
export async function downloadWithFallback(opts: {
  ...
  if (!pathStr || pathStr.startsWith('Download failed')) {
    throw new Error(pathStr || `MCP 下载失败: ${opts.arxivId}`);
  }
